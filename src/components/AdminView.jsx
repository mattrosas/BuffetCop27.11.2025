...
export default function AdminView({ user }) {
  ...
  const puedeEditarMenu = esAdminSupremo || user.rol === 'admin_buffet';
  const puedeEditarSociosYReservas = esAdminSupremo || user.rol === 'admin_cooperadora';

  // Guardados
  // Ahora addMenu soporta edición gracias al cambio en storage.js
  ...
  // Acciones
  const editarMenu = (m) => { setFormData(m); setShowModalMenu(true); }; // <--- NUEVA FUNCIÓN
  const borrarMenu = (id) => { if(window.confirm("¿Borrar menú?")) { Storage.deleteMenu(id); recargarDatos(); } }; // <--- NUEVA FUNCIÓN
      ...
      <div className="dashboard-content">
        <div className="welcome-banner">
          ...
          <div style={{display: 'flex', flexDirection: 'column', gap: '10px'}}>
            ...
            {/* === BOTÓN ADMIN BUFFET (AHORA CREA EMPLEADOS) === */}
                    ...
                    <tbody>
                        {menus.map((m,i)=> (
                            <tr key={i}>
                                ...
                                <td>${m.precio}</td>
                                <td>
                                    {/* Botón EDITAR para Admin Buffet y Supremo */}
                                    {puedeEditarMenu && (
                                        <>
