...
export default function AdminView({ user }) {
  ...
  // PERMISOS BUFFET (Admin Buffet o Empleado Buffet con permiso activo)
  const esEquipoBuffet = user.rol === 'admin_buffet' || (user.rol === 'user_buffet_employee' && user.permisos);
  
  // PERMISOS COOPERADORA (Admin Coop o Empleado Coop con permiso activo)
  const esEquipoCoop = user.rol === 'admin_cooperadora' || (user.rol === 'user_cooperadora' && user.permisos);

  // ¿Quién puede editar qué?
  const puedeEditarMenu = esAdminSupremo || esEquipoBuffet;
  const puedeEditarSociosYReservas = esAdminSupremo || esEquipoCoop;
  ...
  return (
    <div className="dashboard-container">
      <nav className="main-navbar">
        ...
        {user.rol === 'admin_buffet' && <button className={`nav-link ${activeSection === 'personal' ? 'active' : ''}`} onClick={() => setActiveSection('personal')}>👨‍🍳 Personal</button>}
        {user.rol === 'admin_cooperadora' && <button className={`nav-link ${activeSection === 'personal_coop' ? 'active' : ''}`} onClick={() => setActiveSection('personal_coop')}>👥 Empleados</button>}
      </nav>
      ...
      <div className="dashboard-content">
        <div className="welcome-banner">
          <div className="user-info">
            ...
            <div>
              ...
              <p className="user-role">{user.rol.replace('_', ' ').toUpperCase()}</p>
            </div>
          </div>
          <div style={{display: 'flex', flexDirection: 'column', gap: '10px'}}>
            <button onClick={cerrarSesion} className="logout-btn">Cerrar Sesión</button>
            {esAdminSupremo && <><button onClick={() => navigate('/registro')} className="action-btn" style={{fontSize:'12px', padding:'8px 15px'}}>+ Crear Admin Buffet</button><button onClick={() => navigate('/registro-cooperadora')} className="action-btn" style={{fontSize:'12px', padding:'8px 15px', backgroundColor:'#28a745'}}>+ Crear Admin Cooperadora</button></>}
            {user.rol === 'admin_buffet' && <button onClick={() => navigate('/registro-gerente')} className="action-btn" style={{fontSize:'12px', padding:'8px 15px', backgroundColor:'#ffc107', color:'#000'}}>+ Crear Gerente</button>}
            {user.rol === 'admin_cooperadora' && <button onClick={() => navigate('/registro-empleado-cooperadora')} className="action-btn" style={{fontSize:'12px', padding:'8px 15px', backgroundColor:'#17a2b8', color:'#fff'}}>+ Crear Empleado</button>}
          </div>
        </div>
        ...
        {activeSection === 'menus' && <div className="box fade-in"><div className="box-header"><h3>Menús</h3>{puedeEditarMenu && <button className="action-btn" onClick={()=>setShowModalMenu(true)}>+ Nuevo</button>}</div><table><thead><tr><th>Fecha</th><th>Principal</th><th>Precio</th></tr></thead><tbody>{menus.map((m,i)=><tr key={i}><td>{m.fecha}</td><td>{m.principal}</td><td>${m.precio}</td></tr>)}</tbody></table>{!puedeEditarMenu && <p style={{color:'#888', fontStyle:'italic'}}>* Solo lectura.</p>}</div>}
        
        {/* SECCIÓN RESERVAS (Editable por Coop y Admin) */}
        {activeSection === 'reservas' && <div className="box fade-in"><div className="box-header"><h3>Reservas</h3>{puedeEditarSociosYReservas && <button className="action-btn" onClick={abrirNuevaReserva}>+ Nueva</button>}</div><table><thead><tr><th>Socio</th><th>Fecha</th><th>Estado</th><th>Acción</th></tr></thead><tbody>{reservas.map((r,i)=><tr key={i}><td>{r.socio_numero}</td><td>{r.menu_fecha}</td><td><span className={`tag ${r.estado==='Reservado'?'confirmado':'reservado'}`} style={{backgroundColor:r.estado==='Reservado'?'#d4edda':'#f8d7da',color:r.estado==='Reservado'?'#155724':'#721c24'}}>{r.estado||'Reservado'}</span></td><td>{puedeEditarSociosYReservas && <button onClick={()=>editarReserva(r)}>✏️</button>}</td></tr>)}</tbody></table>{!puedeEditarSociosYReservas && <p style={{color:'#888', fontStyle:'italic'}}>* Solo lectura.</p>}</div>}
        
        {/* SECCIÓN SOCIOS (Editable por Coop y Admin) */}
        {activeSection === 'socios' && <div className="box fade-in"><div className="box-header"><h3>Socios</h3>{puedeEditarSociosYReservas && <button className="action-btn" onClick={abrirNuevoSocio}>+ Nuevo</button>}</div><table><thead><tr><th>N°</th><th>Nombre</th><th>DNI</th><th>Estado</th><th>Acción</th></tr></thead><tbody>{socios.map((s,i)=><tr key={i}><td>{s.numero}</td><td>{s.nombre}</td><td>{s.dni}</td><td><span className='tag' style={{backgroundColor:s.estado==='Inactivo'?'#f8d7da':'#d4edda',color:s.estado==='Inactivo'?'#721c24':'#155724'}}>{s.estado||'Activo'}</span></td><td>{puedeEditarSociosYReservas && <><button onClick={()=>editarSocio(s)} style={{marginRight:'5px'}}>✏️</button><button onClick={()=>borrarSocio(s.id)} style={{background:'#dc3545', color:'white', border:'none', borderRadius:'3px'}}>🗑️</button></>}</td></tr>)}</tbody></table>{!puedeEditarSociosYReservas && <p style={{color:'#888', fontStyle:'italic'}}>* Solo lectura.</p>}</div>}
        
        {/* PAGOS (Solo lectura siempre, o editable por coop según lógica futura) */}
